...
namespace SwinAdventure
{
    ...
    {
        [Test]
        public void MoveToPathDestination()
        {
            Player player = new Player("Bach", "The TBF");
            ...
            hogwarts.AddPath(new Path(new string[] { "up" }, godric_valley, "You used a Portkey and flyyy ~"));
            player.Location = hogwarts;
            string announcement = move_command.Execute(player, new string[] { "go", "up" });
            Assert.That(announcement, Is.EqualTo($"You move to up\nYou used a Portkey and flyyy ~\nYou have arrived in Godric Valley"));
            Assert.That(player.Location, Is.EqualTo(godric_valley));
        }
        ...
        public void GetAPath()
        {
            ...
            Location hogwarts = new Location("Hogwarts", "Wizard School");
            Location godric_valley = new Location("Godric Valley", "The hometown of mighty Godric Gryffindor");
            Path godric_path = new Path(new string[] { "up" }, godric_valley, "You used a Portkey and flyyy ~");
            hogwarts.AddPath(godric_path);
            player.Location = hogwarts;
            Path returned_path= move_command.TakePath(player, new string[] { "go", "up" });
            Assert.That(returned_path, Is.EqualTo(godric_path));
        }
        ...
        public void LeaveLocation()
        {
            ...
            hogwarts.AddPath(new Path(new string[] { "up" }, godric_valley, "You used a Portkey and flyyy ~"));
            player.Location = hogwarts;
            string announcement1 = move_command.Execute(player, new string[] { "go", "upppp" });
            string announcement2 = move_command.Execute(player, new string[] { "go", "down" });
            Assert.That(announcement1, Is.EqualTo("Please go to correct direction"));
            Assert.That(announcement2, Is.EqualTo("There is no destination in that direction"));
        }

        [Test]
        public void StayIfTheCommandIsInvalid()
        {
            Player player = new Player("Bach", "The TBF");
            MoveCommand move_command = new MoveCommand();
            ...
            hogwarts.AddPath(new Path(new string[] { "up" }, godric_valley, "You used a Portkey and flyyy ~"));
            player.Location = hogwarts;
            move_command.Execute(player, new string[] { "go", "upppppp" });
            Assert.That(player.Location, Is.EqualTo(hogwarts));
        }
