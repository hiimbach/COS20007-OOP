...
namespace SwinAdventure
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // Command
            CommandProfessor command = new CommandProfessor();
            ...
            hogwarts.Inventory.Put(secret_book);
            godric_valley.Inventory.Put(phoenix_wand);


            Console.WriteLine("Welcome to Swin Adventure");
            Console.WriteLine("You have arrived in Hallway");
            while (true)
            {
                Console.Write("Command -> ");
                string user_input = Console.ReadLine();
                Console.WriteLine(command.Execute(player, user_input));
            }
        }
    }
}
